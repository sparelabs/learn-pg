...
function dedent(text: string): string {
  ...
  const min = lines.reduce(
    (m, l) => Math.min(m, l.match(/^(\s*)/)?.[1].length ?? 0),
    Infinity,
  );
  return text
    .split("\n")
...
export default function LessonPage() {
  ...
  const attemptsReady =
    exercises.length > 0 && attemptQueries.every((q) => q.isSuccess);
  ...
  const latestAttempts = useMemo(() => {
    ...
    for (const q of attemptQueries) {
      ...
      if (attempts && attempts.length > 0) {
        ...
        map[latest.exerciseId] = {
          query: latest.submittedQuery,
          result: parsedResult,
        };
      }
    }
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["attempts", variables.exerciseId],
      });
    },
  });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div
        className={`grid grid-cols-1 ${isMultiSession ? "" : "lg:grid-cols-2"} gap-8`}
      >
        {/* Lesson Content */}
          ...
          <div className="bg-white rounded-lg shadow p-8 lg:sticky lg:top-4 lg:self-start lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto">
            ...
            {currentExercise.setupSql && (
              <div className="mb-4">
                ...
                <details className="mt-3">
                  ...
                  <pre className="mt-2 bg-gray-50 border border-gray-200 rounded p-3 text-xs overflow-x-auto whitespace-pre-wrap text-gray-700">
                    {dedent(currentExercise.setupSql!)}
                  </pre>
                </details>
              </div>
            )}
            ...
            {result && (
              <>
                {/* Validation Feedback */}
                <div
                  className={`mt-6 p-4 rounded ${result.isValid ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
                >
                ...
                {result.queryResults && result.queryResults.rows.length > 0 && (
                  <div className="mt-4 bg-gray-50 border border-gray-200 rounded p-4">
                    <h4 className="font-semibold mb-3">
                      Query Results ({result.queryResults.rowCount} rows)
                    </h4>
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200 text-sm">
                        <thead className="bg-gray-100">
                          <tr>
                            {Object.keys(result.queryResults.rows[0]).map(
                              (col: string) => (
                                <th
                                  key={col}
